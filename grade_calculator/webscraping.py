...
def getCorrectCourseLink(course, mp):
    ...
    WebDriverWait(driver, 4).until(
        EC.presence_of_element_located(
            (By.XPATH, "//*[@id='quickLookup']/table[1]")))
    balls = 0
    ...
    for row in table.find_elements(By.XPATH, ".//tr"):
        if (balls == 0 or balls == 1):
            ...
            continue
        print(row.id)
        if (course in row.find_element(
                By.CLASS_NAME, "table-element-text-align-start").text.lower()):
            print("found it!")
            course_id = row.get_attribute("id")
            if (mp == "Q1"):
                xpath = "//*[@id='" + course_id + "']/td[13]/a"
                quarter_link = driver.find_element(By.XPATH,
...
def getCourseList():
    ...
    for row in table.find_elements(By.XPATH, ".//tr"):
        ...
        try:
            if (row.find_element(
                    By.CLASS_NAME, "table-element-text-align-start"
            ).text.lower().__contains__("guidance") or row.find_element(
                    By.CLASS_NAME, "table-element-text-align-start"
            ).text.lower().__contains__("physical education")
                    or row.find_element(By.CLASS_NAME,
                                        "table-element-text-align-start").text.
                    lower().__contains__("band")):
                continue
            else:
                ogcourses.append(
                    row.find_element(
                        By.CLASS_NAME,
                        "table-element-text-align-start").text.lower())
        except:
            continue
...
def getQuarterAssignmentInfo(link):
    ...
    for row in table.find_elements(By.XPATH, ".//tr"):
        ...
        try:
            category = row.find_element(By.XPATH,
                                        category_location).text.lower()
